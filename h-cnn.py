...
y_c2_validation = np.zeros((y_validation.shape[0], num_c_2)).astype("float32")
...
for i in range(y_c2_validation.shape[0]):
  y_c2_validation[i][parent_f[np.argmax(y_validation[i])]] = 1.0
...
y_c1_validation = np.zeros((y_c2_validation.shape[0], num_c_1)).astype("float32")
...
for i in range(y_c1_validation.shape[0]):
  y_c1_validation[i][parent_c2[np.argmax(y_c2_validation[i])]] = 1.0

#----------------------- model definition ---------------------------
alpha = K.variable(value=0.8, dtype="float32", name="alpha") # A1 in paper
beta = K.variable(value=0.15, dtype="float32", name="beta") # A2 in paper
gamma = K.variable(value=0.05, dtype="float32", name="gamma") # A3 in paper
input_shape=(a, 1)
...
c_1_bch = Dense(128, activation='relu')(c_1_bch)
...
c_2_bch = Dense(128, activation='relu', name='c2_fc_1')(c_2_bch)
#c_2_bch = BatchNormalization()(c_2_bch)
c_2_bch = Dropout(0.5)(c_2_bch)
c_2_bch = Dense(128, activation='relu', name='c2_fc2')(c_2_bch)
#c_2_bch = BatchNormalization()(c_2_bch)
c_2_bch = Dropout(0.5)(c_2_bch)
c_2_pred = Dense(num_c_2, activation='softmax', name='c2_predictions')(c_2_bch)
...
score.to_excel(r"E:\pythonProject3\5\1h-cnnyuce概率.xlsx", index=False, header=False)
#predict_test_1 = np.argmax(predict_test, axis=1)
...
for i in range(len(y_pred)):
    ...
    for j in range(len(y_pred[i])):
        if max_value == y_pred[i][j]:
            ...
            y_pred[i][j] = 0
score = pd.DataFrame(y_pred)
score.to_excel(r"E:\pythonProject3\5\1h-cnnyuce标签.xlsx", index=False, header=False)
...
print("validation score：",model.evaluate(x_validation,  y_validation, verbose=0))
...
plt.plot([0, 1], [0, 1], 'k--', lw=lw,color= 'g')
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
...
plt.savefig(r"E:\pythonProject3\5\1H-CNNroc.png", dpi=200, bbox_inches='tight', transparent=False)
plt.show()
...
def plot_confusion_matrix(cm, classes, title='Confusion matrix', cmap=plt.cm.Blues,normalize=True):
    ...
    plt.savefig(r"E:\pythonProject3\5\1H-CNN混淆.png", dpi=200, bbox_inches='tight', transparent=False)
    plt.show()
