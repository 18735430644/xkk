...
model = SVC(C=1., kernel='poly', degree=2,decision_function_shape='ovr',probability=True)
...
print("PolyTrain_score:{0}\nTest_score:{1}".format(model.score(x_train, y_train), model.score(x_test, y_test)))
print("查准率：",metrics.precision_score(y_pred, y_test,average='macro'))
print("召回率：",metrics.recall_score(y_pred, y_test,average='macro'))
print("F1：",metrics.f1_score(y_pred, y_test,average='macro'))
print('SVM_validation score', model.score(x_validation, y_validation))
...
print(' SVM_test auc_score  is: ', auc1)
